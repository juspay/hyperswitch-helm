...
try:
    ...
    print("Error: vertexai package not found. Install with: pip install google-cloud-aiplatform")
    sys.exit(1)
...
class ClaudeCodeOrchestrator:
    """Orchestrates the development workflow using Claude via Google Vertex AI"""
    ...
    async def _generate_issue(self, feature_description: str) -> Dict:
        ...
        try:
            # Use Claude Code agent through Vertex AI
            ...
            # Parse response to extract issue data
            issue_content = response.text
    ...
    async def _run_dev_agent(self, issue_data: Dict) -> Dict:
        ...
        try:
            ...
            dev_output = response.text
            
            # Commit the changes
            self._commit_changes("feat: implement feature via Claude Code agent")
    ...
    async def _apply_reviewer_feedback(self, implementation: Dict, feedback: str) -> Dict:
        ...
        try:
            ...
            updated_implementation = response.text
...
async def main():
    ...
    # Configuration - Use user's existing Vertex AI setup
    ...
    if not use_vertex:
        print("❌ Error: CLAUDE_CODE_USE_VERTEX not set to 1")
        print("💡 Set it with: export CLAUDE_CODE_USE_VERTEX=1")
        sys.exit(1)
